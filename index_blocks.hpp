...
const size_t HEAD_PL_OFFSET = (4 * sizeof(uint32_t) + 2 * sizeof(uint8_t));
...
struct head_block {
  ...
  uint8_t  m_tail_byte_offset; // next unused byte in the tail block
  ...
  // Tells us where to write next in the tail block
  uint8_t tail_byte_offset() const {
    return m_tail_byte_offset;
  }

  // Explicitly sets the offset in the tail block
  void set_tail_byte_offset(const uint8_t tail_byte_offset) {
    m_tail_byte_offset = tail_byte_offset;
  }

  // Advances the offset by a given stride
  ...
  // Byte offset to first postings entry
  size_t data_offset() {
...
struct torso_block {
  ...
  uint8_t  m_bytes[TT_BYTES]; // variable-byte postings; first one is the b-gap
...
struct tail_block {
  ...
  // Used to set the plain first docid (d_num)
  void set_first_docid(const uint32_t first_docid) {
    m_first_docid = first_docid;
  }

  // Pointer to the memory address of the bytes buffer
  uint8_t* buffer_ptr() {
    return &m_bytes[0];
  }
